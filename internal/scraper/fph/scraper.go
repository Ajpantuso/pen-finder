...
func (s *Scraper) processHREF(href string) (result, error) {
	...
	if !strings.HasPrefix(link, "/") {
		...
		var err error
		link, err = url.JoinPath("https://fountainpenhospital.com", href)
		if err != nil {
	...
	if strings.HasPrefix(url.Path, "/collections/back-room-1/products/") {
		product = strings.TrimPrefix(url.Path, "/collections/back-room-1/products/")
	}
